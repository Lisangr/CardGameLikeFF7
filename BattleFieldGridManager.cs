...
public class BattleFieldGridManager : MonoBehaviour
{
    ...
    public GridCell[,] gridCells;    // Двумерный массив для хранения ячеек
    ...
    void GenerateGrid()
    {
        ...
        gridCells = new GridCell[numberOfColumns, numberOfRows];
        ...
        for (int y = 0; y < numberOfRows; y++)  // Перебор по строкам (y)
        {
            for (int x = 0; x < numberOfColumns; x++)  // Перебор по столбцам (x)
            {
                ...
                gridCells[x, y] = cell.GetComponent<GridCell>(); // Присваиваем ячейку в массив
